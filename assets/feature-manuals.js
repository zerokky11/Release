(function () {
  const staticAssetVersion = '3.0-20260731.1';
  ...
  const features = [
    ...
    },
    {
      ...
      userGuide: '모델 안에 남은 비정상 참조 객체를 정리하는 기능입니다. 명백한 유령객체 다섯 종류는 항상 삭제하고, 추가로 의심되는 네 종류는 설정에서 삭제 여부를 선택합니다. 체크하지 않은 선택 규칙도 검출 결과와 Excel에는 남으므로 먼저 검출만 해 본 뒤 삭제 범위를 결정할 수 있습니다.',
      setup: [
        ...
        '선택 규칙을 끄면 해당 객체를 삭제하지 않고 검출 결과와 Excel에만 기록합니다. 먼저 모두 끈 상태로 검토한 뒤 결과를 확인하고 필요한 규칙만 켜는 방식이 안전합니다.',
      ...
      settingDetails: [
        ...
        {
          label: '검출만 사용',
          description: '선택 규칙을 끄면 삭제는 하지 않고 후보와 근거를 결과 Excel에 기록합니다.',
          example: '첫 실행에서는 네 선택 규칙을 모두 끄고 후보를 검토한 뒤 승인된 규칙만 켜 재실행합니다.'
        }
      ],
      run: [
      ...
      result: [
        ...
        'Excel은 검토 요약, 규칙별 집계, 검출 결과, 실제 삭제, 예상 외 삭제, 실행 중 추가, 삭제 반환 전체, 카테고리 집계 시트로 구성됩니다.',
        '선택 규칙을 끈 후보도 검출 결과 시트에 검출만으로 남아 다음 실행의 삭제 범위를 판단할 수 있습니다.',
        '오류나 예상 외 삭제가 있으면 규칙, ElementId, 카테고리, 패밀리/타입과 메시지를 확인합니다.'
      ],
      export: {
        multi: true,
      ...
      extra: [
        {
          ...
          title: '선택 삭제 범위 확인',
          description: '선택 규칙을 켜면 해당 후보를 삭제하고, 끄면 후보와 근거만 결과 Excel에 기록합니다.',
