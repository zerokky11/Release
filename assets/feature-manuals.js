(function () {
  ...
  const features = [
    ...
    },
    {
      ...
      run: [
        '새 센트럴만 필요하면 센트럴 파일 생성을, 새 센트럴과 입력한 웍셋을 함께 만들면 센트럴 생성 + 웍셋 추가를 실행합니다.',
        '이미 존재하는 센트럴에 웍셋만 넣을 때는 웍셋만 확인·추가 탭으로 전환한 뒤 웍셋 추가 실행을 누릅니다. 일반 RVT는 먼저 새 센트럴을 만들어야 합니다.',
        '실행 전 오른쪽 준비 상태에서 파일 등록, 센트럴 경로, 웍셋 입력의 확인 필요 항목이 없는지 확인합니다.',
        '완료 요약에서 생성 센트럴, 새로 추가된 웍셋, 기존 이름이라 건너뛴 웍셋, 요청한 3D 뷰 권한 수와 파일별 메시지를 확인합니다.'
      ],
      logic: [
        ...
        '새 센트럴 생성 후 기본 정책은 모든 Revit 소유권 반납입니다. 옵션을 켜면 정확히 일치하는 비템플릿 3D 뷰의 ViewWorkset만 현재 사용자 소유로 다시 확보합니다.',
        '패밀리 문서, 클라우드/서버 모델, 현재 Revit보다 높은 형식, 같은 원본·센트럴 경로로의 출력은 실행 전에 제외하거나 실패로 기록합니다.'
      ],
      result: [
        '파일별 센트럴 생성 성공/실패/건너뜀 상태, 실제 출력 센트럴 경로와 새로 만든 웍셋을 확인할 수 있습니다.',
        '기존에 이미 있던 웍셋은 건너뜀으로 구분되고, 3D 뷰 소유권 옵션을 사용한 경우 요청 수와 실제 확보 수를 함께 표시합니다.',
  ...
  const manualEnhancements = {
    ...
    centralworkset: {
      ...
      settingDetails: [
        {
          label: '작업 모드',
          description: '센트럴 파일 만들기는 센트럴·로컬·일반 RVT를 새 파일로 중앙화한다. 웍셋만 확인·추가는 이미 연결된 기존 센트럴에 사용자 웍셋만 추가하며, 일반 RVT나 연결 센트럴을 찾지 못한 복사본은 이 모드에서 실행할 수 없다.',
          example: '일반 모델 A.rvt를 새 센트럴로 만들 때는 센트럴 파일 만들기를 사용한다. 이미 있는 A_Central.rvt에 Coordination 웍셋만 넣을 때는 웍셋만 확인·추가를 사용한다.'
        },
        {
          label: '대상 등록과 파일 상태',
        ...
        {
          label: '새 센트럴 출력 경로',
          description: '센트럴 생성은 원본과 기존 연결 센트럴 경로가 아닌 절대 경로의 .rvt가 필요하다. 공통 출력 폴더를 고르면 등록한 파일명으로 경로를 채우고, 필요한 경우 선택한 파일의 경로만 다시 바꿀 수 있다.',
          example: 'D:\\Central_Output\\A동_설비.rvt처럼 원본과 다른 폴더를 지정한다. C:\\Models\\A동_설비.rvt를 그 자체의 출력 경로로 입력하면 실행 전에 막힌다.'
        },
        {
          label: '추가할 사용자 웍셋',
        ...
        {
          label: '3D 뷰 소유권',
          description: '센트럴 파일 만들기에서만 사용할 수 있는 선택 옵션이다. 켜면 새 센트럴의 모든 소유권을 반납한 뒤, 적은 이름과 대소문자·공백까지 정확히 일치하는 비템플릿 3D 뷰의 ViewWorkset만 현재 Revit 사용자 소유로 다시 확보한다. 끄면 모든 소유권을 반납한다.',
          example: 'Coordination 3D만 작업자가 계속 편집해야 하면 옵션을 켜고 해당 이름을 한 줄에 하나씩 입력한다. 다른 사용자 웍셋과 뷰 권한은 이 옵션으로 보존되지 않는다.'
        },
        {
          label: 'Excel 일괄 설정',
  ...
  const manualScreenOverrides = {
    ...
    centralworkset: {
      base: {
        title: '파일별 센트럴·웍셋 설정',
        description: '등록한 RVT의 유형과 기존 웍셋을 확인하고, 새 센트럴 경로·추가 웍셋·실행 포함 여부를 파일별로 정합니다.',
        points: ['센트럴·로컬·일반 RVT 상태', '새 센트럴 절대 경로', '무제한 사용자 웍셋 입력', '센트럴 생성·웍셋 추가 실행']
      },
      extra: [
        {
          file: 'centralworkset-ownership.png',
          title: '선택 3D 뷰 소유권 유지',
          description: '새 센트럴 생성 뒤 모든 소유권을 반납한 다음, 지정한 비템플릿 3D 뷰의 ViewWorkset만 현재 사용자 소유로 다시 확보하는 선택 옵션입니다.',
          points: ['센트럴 생성에서만 사용', '한 줄에 하나의 정확한 3D 뷰 이름', '나머지 Revit 소유권 반납', '웍셋 단독 실행에는 적용 안 됨']
        }
      ]
    },
    connector: {
