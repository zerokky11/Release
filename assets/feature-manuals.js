(function () {
  const staticAssetVersion = '3.0-20260906.2';
  ...
  const features = [
    {
      ...
      title: '다회차 엑셀 결과 정리',
      badge: 'Excel 작업',
      summary: '이전 회신과 이번 검토 Excel을 연결해 코멘트와 인정된 예외를 반영하고, 최종 조치 대상 수량을 정리합니다.',
      target: '기존 .xlsx/.xls 파일. RVT를 열거나 재검토하지 않습니다.',
      userGuide: '시공에서 답변한 1차 회신과 설계에 전달할 2차 검토 결과가 이미 있을 때 사용합니다. 원본을 읽어 새 정리본을 만들며, Revit 문서를 열지 않은 상태에서도 Hub의 BQC → 결과 정리에서 실행할 수 있습니다.',
      setupLead: '1차 회신 등록 → 2차 결과 등록 → 파일 연결 확인 → 답변·판정·예외 설정 → 정리한 엑셀 저장 순서로 진행합니다.',
      settingDetails: [
        { label: '1차 회신', description: '작업자의 코멘트나 예외 판정이 적힌 이전 회차 Excel을 등록합니다. 파일 선택은 여러 파일, 폴더 선택은 하위 폴더까지 읽습니다. 빈 파일이나 ID가 없는 집계 시트는 비교 대상으로 쓸 수 없습니다.', example: '시공에서 돌려받은 Plant_A_미연결검토_40EA.xlsx를 1차 회신에 등록합니다.' },
        { label: '2차 결과', description: '이번 모델 검토에서 생성한 결과를 등록합니다. 정리본은 이 파일의 시트·제목·병합·서식·수식·참고 시트와 이미 입력한 작업자 답변을 유지하고 비교 열을 추가합니다.', example: '모델 수정 후 생성한 Plant_A_미연결검토_38EA.xlsx를 2차 결과에 등록합니다.' },
        { label: '결과 연결 확인', description: '모델, 검토 종류, 파라미터와 열 구성이 맞는 후보를 확인합니다. 검토 종류를 알 수 없는 일반 양식은 모델명을 제외한 파일명 규칙과 시트명도 같아야 자동 확정합니다. 후보가 여럿이면 해당 항목을 열어 1차 회신과 대상 모델을 직접 지정합니다.', example: '같은 Plant_A라도 미연결 검토와 라인번호 연속성 검토는 서로 연결하지 않습니다.' },
        { label: '같은 파일명 규칙에 연결 적용', description: '수동으로 올바르게 연결한 한 항목을 기준으로 다른 모델의 같은 양식·파라미터·파일명 규칙에도 연결을 적용합니다. 후보가 여럿이거나 이미 직접 수정한 연결은 유지합니다.', example: 'Plant_A_회신과 Plant_A_설계전달을 연결한 뒤 같은 이름 규칙의 Plant_B, Plant_C에도 적용합니다.' },
        { label: '답변 열과 판정 열', description: '1차 파일의 작업자 코멘트 열과 검토 판정 열을 각각 지정합니다. 답변 열은 여러 개를 선택할 수 있고, 제목이 없는 열은 G열 · 제목 없음처럼 열 문자와 실제 내용 예시로 고릅니다. 같은 양식에 답변·판정 열 적용은 열 구조가 같은 파일에만 적용합니다.', example: 'G열의 시공 답변을 답변 열로, H열의 검토 판정을 판정 열로 지정합니다. 두 용도로 같은 열을 고르지 않습니다.' },
        { label: '답변 확인 · 예외 지정', description: '판정 열이 없는 회신은 같은 답변을 묶어 확인한 뒤 합의된 예외를 직접 선택합니다. 완료·반영함 같은 답변만으로는 제외하지 않습니다.', example: '설계상 개방 끝단이라고 답변한 항목 중 예외로 합의한 묶음만 선택합니다.' },
        { label: '오류 수량에서 제외할 키워드', description: '한 줄에 키워드 하나씩 입력합니다. 키워드를 찾을 열은 답변 열과 별도로 지정할 수 있습니다. OR는 하나라도 포함, AND는 같은 행의 선택한 열들에 모든 키워드가 있을 때 후보가 됩니다. 영문 대소문자는 구분하지 않으며 정규식은 사용하지 않습니다.', example: '설계상과 예외를 두 줄에 넣고 AND를 선택하면 같은 행의 G열 설계상, H열 예외도 후보가 됩니다. 서로 다른 행을 합쳐 판단하지 않습니다.' },
        { label: '키워드 제외 미리보기', description: '1차 회신에서 키워드에 해당한 문구와 행 수를 먼저 확인합니다. 키워드나 검색 열이 비면 제외 후보가 생기지 않습니다. 조건을 바꾸면 미리보기를 다시 확인해야 합니다. 이 수량은 2차 결과의 최종 제외 수량과 다를 수 있습니다.', example: '1차에서 38개가 후보여도 이번에 같은 조건·결과로 재검출된 30개만 최종 제외될 수 있습니다.' }
      ],
      run: [
        'BQC → 결과 정리 → 다회차 엑셀 결과 정리를 엽니다.',
        '1차 회신과 2차 결과를 각각 등록하고 확인 필요한 항목만 보기로 미확정 연결을 해결합니다.',
        '답변·판정 열과 키워드 검색 열을 선택하고 필요한 예외만 지정합니다. 답변이 수정 완료여도 이번에 재검출되면 조치 대상으로 남습니다.',
        '정리한 엑셀 저장을 누른 뒤 저장 폴더를 선택합니다. 시각별 새 하위 폴더에 정리본과 결과정리_이력.json을 저장합니다.',
        '파일별 결과와 전체 검출 / 예외 제외 / 최종 조치 대상을 확인합니다. 정리 중단을 누르면 이미 저장된 정리본은 남습니다.'
      ],
      logic: [
        '같은 모델·객체·검토 항목·실제 결과를 기준으로 대응 행을 찾습니다. 새 양식은 검토 설정 서명도 비교하며, 통합 파일은 행별 모델명으로 분리합니다.',
        '명시적인 예외 인정, 사용자가 지정한 예외 또는 키워드 조건에 해당하는 이전 행 중 이번 결과와 조건까지 일치한 항목만 기존 예외로 처리합니다.',
        '값이나 검토 설정이 바뀌거나 회신이 충돌하면 예외를 자동 적용하지 않고 재확인 대상으로 남깁니다. 다른 모델의 같은 ID·그룹 번호를 섞지 않습니다.',
        '완료 답변이 있는 항목을 다시 검출하면 현재 판정은 수정 필요, 다회차 상태는 재검출로 기록합니다.',
        '인식된 시트 중 일부가 빠졌거나 행을 식별하지 못하면 최종 수량을 확정하지 않습니다. ID가 없는 참고·집계 시트는 그대로 보존합니다.',
        '등록 후 원본 파일이 바뀌거나 같은 파일을 1차·2차 양쪽으로 연결하면 다시 선택해야 합니다.'
      ],
      result: [
        '2차 원본 구조에 이전 코멘트·판정, 현재 판정, 다회차 상태, 회신 위치와 예외 적용 근거를 추가합니다.',
        '수량은 파일 안에서 모델·객체·항목·결과·조건이 같은 상세 항목을 중복 제거해 셉니다. 기존 기능의 객체 수나 객체 쌍 수와 단위가 다를 수 있습니다.',
        '38건 모두 기존 예외면 최종 조치 대상 0, 신규 1건이 추가되면 1입니다. 예외 상세 행과 이전 코멘트는 삭제하지 않습니다.',
        '이전에만 있던 항목은 이전 결과 미매칭으로 확인합니다. 이번 범위에서 빠졌을 수 있으므로 수정 완료로 단정하지 않습니다.'
      ],
      export: {
        workflow: true, title: '정리본 저장과 파일명', single: 'Plant_A_미연결검토_01EA.xlsx',
        steps: [
          '기존 Excel 정리이므로 한글/영문 또는 한 파일/파일별로 다시 구성하는 네 가지 출력 옵션을 사용하지 않습니다. 입력한 2차 파일의 언어와 통합·개별 구성을 유지합니다.',
          '최종 수량을 확정하면 파일명 끝에 최소 두 자리 수량을 붙입니다: Plant_A_미연결검토_00EA.xlsx, Plant_A_미연결검토_01EA.xlsx. 100건은 100EA입니다.',
          '같은 이름이 있으면 Plant_A_미연결검토 (2)_01EA.xlsx처럼 수량 앞에 번호를 붙여 기존 파일을 보존합니다.',
          '일부 시트 미정리나 식별 불가로 수량이 불확실하면 00EA로 단정하지 않고 원래 파일명을 유지합니다.'
        ],
        note: '정리본과 처리 이력을 함께 보관합니다. 다음 회차에는 최신 답변·판정이 적힌 회신을 다시 1차 쪽에 등록합니다.'
      },
      excelVisuals: false,
      notes: ['열 설정과 키워드 조건은 다음에 다시 사용할 수 있습니다. 파일 연결과 임시 수동 예외는 다음 작업으로 자동 이전되지 않습니다. 원본 파일은 읽기만 합니다.']
    },
    {
      id: 'repeatreview',
      group: 'BQC 결과 정리',
      title: '여러 RVT 다회차 검토',
      badge: '배치 결과',
      summary: '여러 RVT 검토를 실행하면서 이전 회신을 결과 Excel에 이어 붙이고, 기존 예외를 반영한 조치 수량을 저장합니다.',
      target: '여러 RVT 자동 Excel 저장. 활성 문서 단독 검토는 제외합니다.',
      userGuide: '이번 RVT 검토부터 결과 저장까지 한 번에 진행할 때 사용합니다. 여러 RVT 검토 화면에서 다회차 검토를 켜면 기능별 파일 이름과 이전 회신을 먼저 설정합니다. 이미 생성한 Excel끼리만 정리하려면 다회차 엑셀 결과 정리를 사용합니다.',
      setupLead: '기능 설정 → RVT 등록 → 다회차 검토 켜기 → 기능별 파일명·회신 설정 → 저장하고 검토 시작 순서입니다.',
      settingDetails: [
        { label: '다회차 검토', description: '여러 RVT 실행 화면 또는 RVT 목록 확대 화면에서 켭니다. 실행 버튼을 누르면 결과 엑셀 파일명 설정창이 열립니다. 기존 실행 후 직접 저장을 선택했어도 다회차를 켜면 RVT별 자동 저장으로 실행됩니다.', example: '미연결과 연속성 두 기능을 선택하고 RVT를 등록한 뒤 다회차 검토를 켭니다.' },
        { label: '결과 파일명과 사용 이력', description: '기능별로 파일명에 사용할 이름을 입력하거나 저장한 이름을 고릅니다. RVT명은 유지됩니다. 이름은 기능마다 다르게 쓰고 Windows 파일명 금지 문자를 넣지 않습니다.', example: '미연결은 배관_미연결, 연속성은 라인번호_연속성으로 지정합니다.' },
        { label: '이전 결과에서 찾을 이름', description: '이전 회신 폴더를 읽을 때 파일명을 거르는 이름입니다. 이번에 사용할 결과 이름과 다르면 이전 이름을 따로 입력합니다. 파일 선택으로 직접 등록하는 것도 가능합니다.', example: '이번 이름은 배관_미연결이지만 이전 파일은 접속검토였다면 찾을 이름에 접속검토를 입력합니다.' },
        { label: '이전 회신 폴더·파일', description: '여러 Excel 파일 또는 폴더를 선택합니다. 모델명이 정확히 같은 후보부터 연결하고 부분 이름은 긴 RVT명부터 대조합니다. 후보가 여러 개면 모델을 직접 지정합니다. 첫 회차는 이전 회신을 비워도 됩니다.', example: 'Plant_A.rvt와 Plant_A_접속검토.xlsx를 연결합니다. 통합 Excel은 File 열의 모델명으로 나눠 읽습니다.' },
        { label: '답변 열·판정 열', description: '이전 회신에서 코멘트 열을 하나 이상 선택하고 예외 인정이 적힌 판정 열을 지정합니다. 제목 없는 열, 중간에 추가한 열과 셀 메모도 읽습니다. 같은 열 구조에만 일괄 적용합니다.', example: 'G열과 I열을 답변 열로, H열을 판정 열로 지정합니다.' },
        { label: '예외 지정과 OR/AND 키워드', description: '판정 열이 없는 회신은 답변 확인 · 예외 지정으로 합의된 항목을 고르거나, 기능별 제외 키워드를 한 줄씩 입력합니다. 키워드를 찾을 열을 따로 고르고 OR 또는 AND 조건을 선택합니다. AND는 같은 행 안에서 모든 키워드가 있어야 합니다.', example: '설계상 / 예외 두 키워드와 AND를 지정한 후 미리보기에서 실제 1차 회신 후보를 확인합니다.' },
        { label: '저장 방식', description: '기능별 통합 또는 RVT별 저장의 결과마다 최종 조치 수량을 계산합니다. 여러 개 등록한 파라미터 연속성은 통합 모드에서도 RVT·등록 항목별 파일을 만듭니다.', example: 'KKY_LineNo와 KKY_System을 따로 등록하면 같은 RVT에서도 검토01, 검토02 파일이 나뉩니다.' }
      ],
      run: [
        '기능과 각 설정을 먼저 완성하고 여러 RVT 검토에서 대상을 등록합니다.',
        '다회차 검토를 켜고 검토 시작을 누릅니다. 파일명 설정창에서 기능별 이름을 확인하고 이전 회신을 연결합니다.',
        '답변·판정·키워드 검색 열과 예외 조건을 정합니다. 확인 필요 상태의 모델 연결을 모두 해결합니다.',
        '저장하고 검토 시작을 누르고 저장 폴더를 선택합니다. 첫 회차는 회신 없이도 실행할 수 있습니다.',
        '실행 후 시각별 결과 폴더에서 기능 결과, 다회차_검토요약.xlsx와 해당 회차 설정·요약 JSON을 확인합니다.'
      ],
      logic: [
        '현재 검토 결과를 만든 뒤 이전 회신의 모델·객체·검토 항목·결과·조건과 비교합니다. 새 파일은 숨겨진 식별 정보와 검토 설정 서명을 함께 사용합니다.',
        '이전 판정이 예외 인정이거나 명시적으로 선택한 예외·키워드 후보이며 이번 결과까지 일치하면 기존 예외로 분류합니다.',
        '이전 답변이 완료여도 이번에 다시 검출되면 수정 필요로 기록합니다. 회신 충돌이나 값·설정 변경은 보류와 재확인 대상으로 남깁니다.',
        '기존 양식도 ID와 결과 열로 읽습니다. 비교 가능한 열이 달라졌다면 코멘트만 참고로 남기고 자동 제외하지 않습니다.',
        '집계·기준 시트, 좌표 추출 및 객체 ID가 아닌 규격 ID를 쓰는 PMS 표에는 같은 행 비교를 적용하지 않습니다. 좌표/PMS에는 파일명 설정만 적용됩니다.',
        '파일별로 상세 항목을 중복 제거해 수량을 계산합니다. 일부 저장 실패나 인식 불가 결과를 0건으로 간주하지 않습니다.'
      ],
      result: [
        '다회차 결과와 요약 Excel에는 검출 항목 / 기존 예외 / 조치 대상 / 코멘트 반영 / 재확인을 구분해 표시합니다.',
        '기능 파일의 첫 수량 요약 시트에는 전체 검출 / 예외 제외 / 최종 조치 대상을 기록합니다. 파일명 수량은 최종 조치 대상과 같습니다.',
        '각 상세 행에는 이전 코멘트·판정, 새 답변·현재 판정, 다회차 상태와 회신 위치가 남습니다. 키워드 제외는 조건·키워드·이전 열 문자를 근거로 기록합니다.',
        '이전에만 있던 결과는 이전 결과 미매칭으로 별도 확인하며 자동 완료 처리하지 않습니다.'
      ],
      export: {
        workflow: true, title: '자동 Excel 저장과 파일명', single: '{RVT명}_{설정한 기능명}_01EA.xlsx',
        steps: [
          '한글 + RVT별: Plant_A_배관_미연결_01EA.xlsx처럼 저장합니다. 기존 Excel 언어 설정이 한글이면 기능 결과의 헤더도 한글입니다.',
          '영문 + RVT별: Plant_A_Unconnected_01EA.xlsx처럼 파일명 설정창에 입력한 이름을 사용합니다. 영문 언어 설정이 사용자 지정 이름을 자동 번역하지는 않습니다.',
          '한글 + 기능별 통합: 기능별 통합 파일 하나에 여러 RVT 결과를 순차 기록하며 그 통합 파일의 최종 조치 수량을 끝에 붙입니다.',
          '영문 + 기능별 통합: 통합 구성은 같고 기능 결과의 언어가 달라집니다. 다회차에서 추가하는 상태·수량 열은 현재 한글 명칭으로 표시됩니다.',
          '여러 연속성 등록은 통합 모드에서도 RVT·파라미터·검토 번호별 파일입니다. 예: Plant_A_라인번호_연속성_KKY_LineNo_검토01_01EA.xlsx. 실제 이름의 공백과 구분자는 선택한 기능명에 따릅니다.',
          '최종 0건은 00EA, 1건은 01EA, 100건은 100EA입니다. 이름 충돌 시 (2)를 수량 앞에 넣고, 수량을 확정할 수 없는 파일은 기존 이름을 유지합니다.'
        ],
        note: '다회차를 끄면 기존 검토와 직접 내보내기 흐름으로 돌아갑니다. 활성 문서 단독 실행은 다회차 자동 저장 대상이 아닙니다.'
      },
      excelVisuals: false
    },
    {
    ...
    },
    {
      ...
      target: '대상 패밀리가 로드된 프로젝트 문서에서 실행합니다. Revit의 관리 > 공유 매개변수에 TXT가 연결되어 있어야 합니다.',
      setup: [
      ...
      logic: [
        ...
        '현재 문서에서 하위 패밀리의 실제 공유 여부를 조사합니다. 다른 프로젝트에서 조사한 같은 숫자 ID의 공유 상태를 재사용하지 않습니다.',
        '하위 패밀리부터 처리한 뒤 프로젝트에 다시 로드하고 상위 패밀리의 파라미터를 연동합니다. 연동과 검증은 현재 부모의 직접 하위만 대상으로 합니다.',
        '예를 들어 Root → Middle → Leaf 구조에서는 Root–Middle, Middle–Leaf 두 관계를 검사합니다. Root–Leaf의 직접 연동이 없다는 이유로 정상 간접 전달을 실패로 세지 않습니다.',
        '같은 문서에서 재실행하면 이미 올바르게 연동된 파라미터를 유지하고 검증합니다. 신규 추가·연동이 0건이어도 기존 연동의 검증 결과를 함께 확인합니다.',
        'Dummy 제외 옵션이 켜져 있으면 해당 하위 패밀리는 처리하지 않습니다. 정상적인 적용 대상 없음과 조사·적용·검증 실패를 구분하고, 프로젝트 로드가 끝난 뒤 반영 수를 집계합니다.'
      ],
      result: [
      ...
      export: {
        multi: false,
        single: 'ParamProp_yyMMdd_HHmmss.xlsx',
        splitKo: '',
        splitEn: '',
        note: '한글·영문 선택과 관계없이 기본 파일명은 ParamProp_yyMMdd_HHmmss.xlsx입니다. 예: ParamProp_260915_093000.xlsx. 단일 실행 보고서이며 여러 RVT의 통합/파일별 분리는 제공하지 않습니다. 저장 창에서 이름을 변경할 수 있습니다.'
      },
      notes: [sharedParamNote]
    },
    {
  ...
  const manualEnhancements = {
    ...
    paramprop: {
      userGuide: '프로젝트에 로드된 복합 패밀리를 조사해 선택한 공유파라미터를 추가하고, 직접 부모와 하위 패밀리 사이의 연동을 구성합니다.',
      setup: [
        ...
        '대상 패밀리가 로드된 프로젝트를 활성화하고, 변경 전 RVT를 별도로 보관합니다.'
      ],
      run: [
        '추가할 공유파라미터를 1개 이상 선택하면 연동 실행 버튼이 활성화됩니다.',
        '연동 실행은 선택한 프로젝트의 대상 패밀리를 하위부터 처리하고, 프로젝트에 다시 로드해 직접 부모와 하위의 파라미터를 연결합니다.',
  ...
  const manualDetailAdditions = {
    ...
    paramprop: {
      userGuide: '프로젝트에 로드된 복합 패밀리에 공유파라미터를 추가하고 직접 부모–하위 연동을 구성합니다. 프로젝트 안의 패밀리가 실제로 변경되므로 실행 전 원본 RVT를 별도로 보관하세요.',
      ...
      run: [
        '처리할 패밀리가 로드된 프로젝트를 활성화합니다. Revit 관리 > 공유 매개변수에서 사용할 TXT를 연결한 뒤 KKY Tool의 공유파라미터 목록을 확인합니다.',
        '파라미터와 적용 범위, 바인딩, Parameter Group, 저장 옵션을 설정합니다.',
        '연동 실행을 누르면 대상 패밀리를 조사하고 하위부터 파라미터 추가·프로젝트 재로드·부모 연동을 처리합니다. 여러 호스트 문서가 열려 선택 창이 나오면 실제로 변경할 프로젝트를 선택합니다.',
        '완료 창의 상태를 확인한 뒤 원문 보고서에서 스캔·적용 대상·프로젝트 반영·추가·연동과 검증 수를 확인합니다. 오류 행이 0개라는 사실만으로 실제 변경 여부를 판단하지 않습니다.',
        '적용 대상이 없으면 취소 상태와 이유가 표시됩니다. 공유 하위 구성, Dummy 제외, 단일 패밀리 옵션을 확인하세요. 실패인 경우 보고서의 패밀리명과 원인을 먼저 확인합니다.',
        '엑셀 내보내기에서 언어와 열 너비 옵션을 고르고 저장 경로를 지정합니다. 보고서 또는 상세 행이 있으면 오류 없는 정상 결과도 저장할 수 있습니다.'
      ],
      result: [
        '완료 창은 완료·취소·실패·실행 불가를 구분합니다. 원문 보고서에서 실제 프로젝트 반영 수와 직접 연동 검증 결과를 확인합니다.',
        '이미 정상 연동된 모델을 다시 실행하면 신규 추가·연동은 0건일 수 있습니다. 기존 연동의 검증 결과와 함께 판단하세요.',
        '상세 목록은 오류·건너뜀·검토 필요 항목을 중심으로 표시합니다. 목록이 비어 있어도 처리 보고서가 있으면 엑셀 내보내기는 가능합니다.',
        'Excel은 보고서 문장을 먼저 기록하고 상세 행을 이어서 저장합니다. 처리 종류, 패밀리, 상세 내용 열에서 각 단계의 결과와 실패 원인을 확인할 수 있습니다.',
  ...
  const continuity = features.find(feature => feature.id === 'connector');
  continuity.settingDetails.push(
    { label: '선택한 파라미터를 각각 등록', description: '선택 파라미터마다 독립 검토 항목을 만듭니다. 등록 당시의 허용 범위·단위·보조 열·제외 옵션·값 매핑을 복사하며 각 항목에서 개별 수정할 수 있습니다. 등록 목록이 있으면 그 목록만 실행하고, 모두 삭제하면 위쪽 기본 설정으로 한 번 실행합니다.', example: 'KKY_LineNo는 1 mm, KKY_System은 2 mm로 별도 등록해 한 번의 여러 RVT 작업에서 검토합니다.' },
    { label: '한쪽이라도 값이 비어 있으면 결과에서 제외', description: '켜면 비교 쌍의 한쪽 또는 양쪽이 빈 결과를 제외합니다. 공백도 빈 값으로 보지만 숫자 0은 남깁니다. 검토 실패와 오류 없음 안내는 숨기지 않으며 활성 문서와 여러 RVT에 모두 적용됩니다.', example: 'A쪽이 빈 문자열이고 B쪽이 CW이면 결과에서 제외합니다. A쪽 0, B쪽 1은 비교합니다.' }
  );
  continuity.run.splice(2, 0, '파라미터별로 기준을 달리할 때는 선택한 파라미터를 각각 등록한 뒤 각 항목의 허용 범위·단위·제외 옵션을 확인합니다. 같은 파라미터를 여러 기준으로 등록할 수도 있습니다.');
  continuity.logic.push('빈 값 제외가 켜져 있으면 공백을 포함한 빈 비교 값을 결과 수집 단계에서 거릅니다. 개별 등록은 등록 당시 복사한 매핑과 각 항목의 조건을 사용합니다.');
  continuity.result.push('여러 RVT에서 여러 개 등록한 연속성은 RVT·등록 항목별 Excel로 저장하며, 통합 자동 저장을 선택해도 이 항목들은 개별 파일입니다.');
  continuity.export.note += ' 여러 개 등록한 검토의 자동 저장에는 파라미터와 검토01·검토02 번호를 붙여 구분합니다. 다회차를 켜면 기능별 사용자 이름 뒤에 같은 구분을 붙이고 최종 조치 수량을 기록합니다.';
  features.find(feature => feature.id === 'deliverycleaner').group = 'BQC 결과 정리';
  ...
  const manualScreenOverrides = {
    revieworganizer: {
      base: { title: '1차 회신과 2차 결과 등록', description: '현재 Hub 전체 화면에 예시 파일을 연결한 모습입니다. 원본을 직접 편집하지 않고 결과 연결과 제외 기준을 먼저 확인합니다.', points: ['1차는 이전 회신', '2차는 이번 결과', '확인 필요한 연결 해결', '원본 보존'] },
      extra: [{ file: 'revieworganizer-settings.png', title: '답변 열·키워드·연결 설정', description: '답변·판정 열과 키워드 검색 열은 서로 다른 용도로 선택합니다. 조건과 미리보기를 확인한 뒤 정리본을 저장합니다.', points: ['제목 없는 열의 예시 확인', 'OR 또는 AND', '미리보기는 이전 회신 후보 수', '같은 양식에 일괄 적용'] }]
    },
    repeatreview: {
      base: { title: '검토 전 다회차 설정', description: '여러 RVT 검토 시작 시 열리는 파일명·회신 설정창입니다. 샘플 회신을 사용한 화면이며 실제 목록은 선택한 기능과 파일에 따라 달라집니다.', points: ['기능별 파일명', '이전 회신 등록', '답변·판정 열', '저장하고 검토 시작'] },
      extra: [{ file: 'repeatreview-columns.png', title: '회신별 열 선택', description: '회신을 펼치고 아래로 스크롤해 답변·판정·키워드 검색 열을 지정합니다. 같은 양식에만 일괄 적용합니다.', points: ['열 문자와 내용 예시', '답변 열은 여러 개 선택', '별도 판정 열', '키워드 검색 열'] }]
    },
    cadcircle: {
      base: {
    ...
    connector: {
      ...
      extra: [
        {
          file: 'connector-registrations.png',
          title: '파라미터별 독립 등록',
          description: '선택한 파라미터를 각각 등록하고 각 항목의 허용 범위·단위·빈 값 제외를 따로 설정합니다.',
          points: ['등록 목록이 있으면 목록만 실행', '항목별 설정', '추가 추출·제외 옵션', 'RVT·등록별 Excel']
        },
        {
  ...
  function renderFeatureIndex() {
    ...
    const groups = ['모델링', 'BQC 검토', 'BQC 결과 정리', '유틸리티'];
    groups.forEach((group) => {
      ...
      const groupCopy = group === '모델링'
        ...
        : group === 'BQC 검토'
          ...
          : group === 'BQC 결과 정리'
            ? ['REVIEW RESULTS', 'BQC 결과 정리', '이전 회신과 이번 검토 결과를 연결하거나 납품 RVT를 정리합니다.']
